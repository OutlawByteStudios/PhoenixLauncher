...
namespace PhoenixLauncher
{
    class HTTPRequest
    {
        ...
        public async Task download(string url, string filename, bool checkDir = true)
        {
            try
            {
                ...
                using (var response = await this.client.GetAsync(url))
                {
                    ...
                    if (!Directory.Exists(Path.GetDirectoryName(filename)) && checkDir)
                    {
            ...
            {
                Debug.WriteLine("Failed to load the file: " + ex.Message);
            }
        }
    }
}
